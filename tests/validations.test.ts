import { isAddressValid, isDecimalBalanceValid } from '../src/utils/validations';
...
describe('Validations', () => {
    ...
    test('wrong balances aren\'t passed', async () => {
        ...
        for (const wrongBalance of wrongBalances) {
            expect(isDecimalBalanceValid(wrongBalance.balance, wrongBalance.decimals)).toBe(false);
        }
    });
    ...
    test('correct balances are passed', async () => {
        ...
        for (const correctBalance of correctBalances) {
            ...
            expect(isDecimalBalanceValid(correctBalance.balance, correctBalance.decimals)).toBe(true);
        }
    });
});
