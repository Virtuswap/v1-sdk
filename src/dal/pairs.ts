...
export async function getAllPairs(): Promise<Array<Pair>> {
    ...
    return pairs.data.pairs.map((pair: any) => {
        ...
        return new Pair(new Address(pair.id),
                 ...
                 pair.reserves.map((reserve: any) => new PairReserve(TokenWithBalance.fromDecimal(pairToken0, reserve.baseBalance), TokenWithBalance.fromDecimal(new Token(reserve.token.id, reserve.token.decimals), reserve.balance))));
    });
}
